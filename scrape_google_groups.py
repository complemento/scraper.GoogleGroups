# Forked manually from https://gist.github.com/punchagan/7947337
# since It's not possible to fork to orgs repository at this time
#
# It's a really any time deprecatable code, since Google seems to want to avoid this kind
# of praxis (scraping) on Google Groups, and it will probably change the way it deal with 
# HTML, Javascript and so
...
##### Some definitions you should change for your case:
forum            = 'https://groups.google.com/forum/#!forum/myforum'

# the main container of all threads on frontpage
threadsXpath     = '//table[@role="list"]/tbody/tr'

# The container of each thread on forums front page
listOfPostsClass = 'F0XO1GC-p-o'
...
class GoogleGroupsScraper(object):
    ...
    def _get_last_post(self):
        ...
        last_post = post_list.find_elements(By.XPATH, threadsXpath)[-1]
    ...
    def _get_post_list(self):
        ...
        return self.driver.find_element_by_class_name(listOfPostsClass)
